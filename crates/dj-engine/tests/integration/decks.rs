...
use dj_engine::decks::{
    led, DeckArm, DeckTransition, MasterBus, SlotControl, DECKS_ID, DEFAULT_SURFACE_CHANNEL,
    EQ_MAX, LEVEL_MAX, LEVEL_UNITY, MOMENTARY_RELEASE_SECS, SLOTS,
};
...
// ---- V2: two arrangements on one clock (the Decks V2 page) ----------

/// A started V2 bank with BOTH pairs wired: the room on the live pair,
/// the headphones on the monitor one.
fn v2_bank() -> Engine {
    let mut e = bank();
    e.add_module("mon1", MONITOR_OUT_ID).unwrap();
    e.connect("bank1", "mon_l", "mon1", "l").unwrap();
...
#[test]
fn a_v2_bank_plays_each_side_through_its_own_fader_and_mute() {
    let mut e = v2_bank();
    load(&mut e, 0, 4); // `load` leaves the (monitor) side muted

    // Both sides muted: both pairs silent.
    e.render_offline(4_096).unwrap();
    let room = e.render_offline(24_000).unwrap().remove(0);
    assert!(peak(&room) < 1e-4, "both sides muted, the room is silent");

    // The MONITOR side unmuted sounds in the headphones and not the room.
    e.decks_set_control("bank1", 0, SlotControl::Mute, 0.0)
        .unwrap();
    e.render_offline(4_096).unwrap();
    let cue = e.render_offline_monitor(24_000).unwrap().remove(0);
    assert!(
        (peak(&cue[12_000..]) - 5.0).abs() < 0.2,
        "the monitor arrangement is the headphones' ({})",
        peak(&cue[12_000..])
    );
    let room = e.render_offline(24_000).unwrap().remove(0);
    assert!(
        peak(&room[12_000..]) < 1e-3,
        "the room still plays the LIVE side, which is muted"
    );

    // The LIVE side has its own fader: unmuted at half, the room hears
    // the same clip at half while the headphones stay where they were.
    e.decks_set_live_control("bank1", 0, SlotControl::Mute, 0.0)
        .unwrap();
    e.decks_set_live_control("bank1", 0, SlotControl::Level, 0.5)
        .unwrap();
    e.render_offline(4_096).unwrap();
    let room = e.render_offline(24_000).unwrap().remove(0);
    assert!(
        (peak(&room[12_000..]) - 2.5).abs() < 0.2,
        "the live side's own fader scales the room ({})",
        peak(&room[12_000..])
    );
}

#[test]
fn a_jump_swaps_the_room_onto_the_monitor_arrangement_at_the_cycle_seam() {
    let mut e = v2_bank();
    load(&mut e, 0, 2); // one loop = 2 beats = 1 s at 120 BPM
                        // Two different arrangements of the same clip: the room at full
                        // fader, the monitor copy at half.
    e.decks_set_live_control("bank1", 0, SlotControl::Mute, 0.0)
        .unwrap();
    e.decks_set_control("bank1", 0, SlotControl::Mute, 0.0)
        .unwrap();
    ...
    e.decks_transition("bank1", DeckTransition::Jump).unwrap();
    let st = e.decks_status("bank1").unwrap();
    assert_eq!(st.transition, DeckTransition::Jump);
    assert!(!st.transition_done, "armed is not fired");

    // The whole first pass still plays the LIVE side.
    let out = e.render_offline(48_000).unwrap().remove(0);
    assert!(
        (peak(&out[24_000..47_000]) - 5.0).abs() < 0.2,
        "the room keeps the live side until the loop's right edge ({})",
        peak(&out[24_000..47_000])
    );

    // Past the seam the room is the monitor arrangement.
    let out = e.render_offline(24_000).unwrap().remove(0);
    assert!(
        (peak(&out[12_000..]) - 2.5).abs() < 0.2,
        "after the seam the room plays what was in the monitor ({})",
        peak(&out[12_000..])
    );
    let st = e.decks_status("bank1").unwrap();
    assert!(st.transition_done, "the jump has fired and owes its commit");
    assert!(st.xfade > 0.99, "the blend sits on the monitor side");

    // The commit copies monitor -> live and rests the blend, changing
    // nothing audible.
    assert!(e.decks_transition_commit("bank1").unwrap());
    let st = e.decks_status("bank1").unwrap();
    assert_eq!(st.transition, DeckTransition::None);
    assert!(!st.transition_done);
    assert!((st.slots[0].live_level - 0.5).abs() < 1e-6);
    assert!(!st.slots[0].live_mute);
    assert_eq!(st.slots[0].live_phase, st.slots[0].phase);
    let out = e.render_offline(24_000).unwrap().remove(0);
    assert!(
        (peak(&out[12_000..]) - 2.5).abs() < 0.2,
        "the committed copy sounds exactly like the blend it replaced ({})",
        peak(&out[12_000..])
    );
    assert!(
        !e.decks_transition_commit("bank1").unwrap(),
        "nothing owed twice"
    );
}

#[test]
fn a_crossfade_blends_over_one_whole_cycle_and_then_reports_done() {
    let mut e = v2_bank();
    load(&mut e, 0, 2);
    e.decks_set_live_control("bank1", 0, SlotControl::Mute, 0.0)
        .unwrap();
    ...
    e.decks_set_control("bank1", 0, SlotControl::Level, 0.5)
        .unwrap();
    e.decks_transition("bank1", DeckTransition::Crossfade)
        .unwrap();

    // The fade waits for the seam: the first pass is all live side.
    let out = e.render_offline(48_000).unwrap().remove(0);
    assert!((peak(&out[24_000..47_000]) - 5.0).abs() < 0.2);

    // Half a cycle into the fade the room sits halfway between the two
    // arrangements (a linear blend of correlated material).
    let out = e.render_offline(24_000).unwrap().remove(0);
    let mid = peak(&out[22_000..]);
    assert!(
        (mid - 3.75).abs() < 0.2,
        "halfway through the fade the blend is half of each ({mid})"
    );
    assert!(!e.decks_status("bank1").unwrap().transition_done);

    // A cycle after it started, the fade is done and the room is the
    // monitor arrangement.
    let out = e.render_offline(30_000).unwrap().remove(0);
    assert!((peak(&out[26_000..]) - 2.5).abs() < 0.2);
    let st = e.decks_status("bank1").unwrap();
    assert!(st.transition_done);
    assert!(e.decks_transition_commit("bank1").unwrap());
}
...
#[test]
fn a_v2_transition_can_be_taken_back_before_it_fires() {
    let mut e = v2_bank();
    load(&mut e, 0, 2);
    e.decks_set_live_control("bank1", 0, SlotControl::Mute, 0.0)
        .unwrap();
    e.decks_transition("bank1", DeckTransition::Jump).unwrap();
    e.decks_transition("bank1", DeckTransition::None).unwrap();
    // Two whole cycles later nothing has fired and the room still plays
    // the live side.
    let out = e.render_offline(96_000).unwrap().remove(0);
    assert!((peak(&out[90_000..]) - 5.0).abs() < 0.2);
    let st = e.decks_status("bank1").unwrap();
    assert_eq!(st.transition, DeckTransition::None);
    assert!(!st.transition_done);
}
...
#[test]
fn the_live_side_shift_is_its_own_and_wraps_like_the_monitors() {
    let mut e = v2_bank();
    load(&mut e, 0, 4);
    e.decks_set_live_phase("bank1", 0, 5).unwrap();
    let st = e.decks_status("bank1").unwrap();
    assert_eq!(st.slots[0].live_phase, 1, "wrapped inside one loop");
    assert_eq!(st.slots[0].phase, 0, "the monitor side did not move");
    e.decks_set_phase("bank1", 0, 2).unwrap();
    let st = e.decks_status("bank1").unwrap();
    assert_eq!(st.slots[0].live_phase, 1, "and vice versa");
    assert_eq!(st.slots[0].phase, 2);
}

#[test]
fn v2_rides_in_the_patch_and_a_classic_bank_keeps_its_bytes() {
    // A bank that has never been V2 serializes exactly as before.
    let classic = serde_json::to_value(dj_engine::decks::DecksState::default()).unwrap();
    assert!(classic.get("v2").is_none());
    assert!(classic["slots"][0].get("live_level").is_none());
    assert!(classic["slots"][0].get("live_mute").is_none());
    assert!(classic["slots"][0].get("live_phase").is_none());

    let dir = tempfile::tempdir().unwrap();
    let mut e = v2_bank();
    load(&mut e, 0, 4);
    e.decks_set_live_control("bank1", 0, SlotControl::Level, 0.75)
        .unwrap();
    e.decks_set_live_control("bank1", 0, SlotControl::Mute, 0.0)
        .unwrap();
    e.decks_set_live_phase("bank1", 0, 3).unwrap();
    e.decks_transition("bank1", DeckTransition::Jump).unwrap();
    e.save_patch(&dir.path().join("p"), "decks-v2").unwrap();

    let e2 = Engine::load_patch(&dir.path().join("p"), crate::common::registry()).unwrap();
    let st = e2.decks_status("bank1").unwrap();
    assert!(st.v2, "a V2 bank comes back a V2 bank");
    assert!((st.slots[0].live_level - 0.75).abs() < 1e-6);
    assert!(!st.slots[0].live_mute);
    assert_eq!(st.slots[0].live_phase, 3);
    assert_eq!(
        st.transition,
        DeckTransition::None,
        "a pending jump is transport, and dies with the session"
    );
}
